...
class RandomDelayWrapper(gym.Wrapper):
    ...
    def __init__(self, env, obs_delay_range=range(0, 8), act_delay_range=range(0, 2), initial_action=None, skip_initial_actions=False):
        ...
        self.delay = max(obs_delay_range) + max(act_delay_range)
        self.use_stochastic_delay = True
        
        self.min_delay = min(obs_delay_range) + min(act_delay_range)
        self.delay = self.min_delay
        self.max_delay = max(obs_delay_range) + max(act_delay_range)
...
class DiscreteActionWrapper(gym.ActionWrapper):
    def __init__(self, env, n_discrete_actions=50):
        super().__init__(env)
        self.n_discrete_actions = n_discrete_actions
        self.action_space = Discrete(n_discrete_actions)
        self.action_values = np.linspace(-env.action_space.high[0], env.action_space.high[0], n_discrete_actions)
...
class NormalizeActionWrapper(gym.ActionWrapper):
    def __init__(self, env):
        ...
        self.action_space = Box(-np.ones_like(self.shift), np.ones_like(self.shift), dtype=np.float32)

    def action(self, action):
        action = action / 2 + 0.5  # Rescale to 0 < a < 1
        action = action * self.scale + self.shift  # Rescale and shift to original action space range
        return action
def create_wrapped_env(env_name='Pendulum-v0'):
    env = gym.make(env_name)
    env = RandomDelayWrapper(env,range(0, 1),range(0, 1))
